...
function determineBump(commits: string[]): BumpType {
  if (commits.some(c => c.startsWith('BREAKING CHANGE') || c.includes('!'))) return 'major';
...
const current  = execSync('git describe --tags --abbrev=0 2>/dev/null || echo v0.0.0', { encoding: 'utf8' }).trim();
