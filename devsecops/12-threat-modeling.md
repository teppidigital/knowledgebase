...
**Threat Modeling** is a structured process for identifying potential security threats to a system *before* it is built or changed. By systematically enumerating threats at design time, teams can design mitigations early — when they are least expensive to implement.
...
function analyzeThreatModel(modelPath: string): void {
  const model: ThreatDragonModel = JSON.parse(fs.readFileSync(modelPath, 'utf-8'));

  const allThreats: ThreatDragonThreat[] = model.detail.diagrams.flatMap(
    d => d.cells.flatMap(c => c.threats ?? [])
  );

  const openThreats = allThreats.filter(t => t.status === 'Open');
  const criticalOpen = openThreats.filter(t => t.severity === 'Very High' || t.severity === 'High');
